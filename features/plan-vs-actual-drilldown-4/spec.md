...
- **Observed-actual series** — the recorded weekly cumulative actual sell-through to date (`actualCurve`, length `weeksElapsed`, last entry equal to the *original* `actualCumulativeFraction`). The weeks that have actually happened. May be absent on a hand-built CC.
...
- **Divergence** — the live current point's value differs from where the observed-actual series sits at the current week (because an F2 edit moved `actualCumulativeFraction` or `weeksElapsed` without rewriting history). When divergent, the final segment of the actual line connecting observed history to the live current point is the *hypothetical* part of the path.
...
- **Edit moves `weeksElapsed`.** The current-week marker and the live current point's *week* move; `actualCurve` length is unchanged, so the live current week may sit beyond or short of observed history. The solid history covers the weeks it has; the segment from the last observed week to the live current point is the dashed hypothetical. No throw, no non-finite coordinate.
- **Single observed week (`weeksElapsed` clamped to 1).** The observed-actual series has one point; there is no solid multi-point history to draw, only the live current point. Renders without error.
...
- an **observed-actual series**: one point per observed week with value `actualCurve[week-1]` when `actualCurve` is present (length = `weeksElapsed` of the *recorded* history), else empty;
- a **live current point**: `{ week: weeksElapsed, value: actualCumulativeFraction }` read from the working CC — the engine-live anchor;
- a **divergent** flag: true iff the live current point does not coincide with the observed-actual series at the current week (value differs, or the current week lies beyond the observed history) — i.e. an edit has moved the live position off the recorded path.
- Properties: every coordinate finite and in range (weeks ≥ 1; values in `[0,1]`); never throws, including when `actualCurve` is absent or when `weeksElapsed` has been edited away from the recorded history length. Deep-equal CC ⇒ deep-equal `Trajectory`.
...
| US-3 observed history solid; divergent post-edit segment distinct; none when not divergent | `trajectory.test.ts` (`divergent` false on a fresh generated CC where live point = observed endpoint; true after `applyEdit` moves `actualCumulativeFraction`; true when `weeksElapsed` edited past observed history) |
| US-4 no `actualCurve` ⇒ plan + live point only, no throw / no non-finite | `trajectory.test.ts` (CC without `actualCurve`: empty observed series, finite live point, no throw) |
...
*(populated by Stage 4 — independent clean-context gate)*
